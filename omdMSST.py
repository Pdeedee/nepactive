...
class omdMSST(MolecularDynamics):
    ...
    def propagate_gammadot(self):
        ...
        # self.iter+=1
        # stress1 = self.atoms.get_stress(voigt=False)[0,0]/units.GPa
        # stress2 = self.atoms.get_stress(voigt=True,include_ideal_gas=True)[0]/units.GPa
        # if self.iter%2:
        #     # print(f"centorofmass:{np.square(self.get_center_of_mass_momentum()).sum()}")
        #     print(f"cell_length_a:{self.cell_length_a:.7f},stress={stress[0,0]/units.GPa:.4f},stress1={stress1:.4f},stress2={stress2:.4f},"
        #             f"stress_m={np.trace(-stress@X):.4f},p0={-np.trace(p0*I@X):.4f},v_shock={np.trace(-np.power(v_shock,2)*totmass/V0*(1-V/V0)*I@X):.4f},"
        #             f"A={A:.4f},X00={X[0,0]:.4f}\n,X={X}",flush=True)
        # #####
        # print(self.gammadot)
        # #####
        self.gammadot = self.gammadot + dthalf*A
        # print(f"a_speed_gamma={A}")

    def propagate_qdot(self):
        forces = self.atoms.get_forces()
        # forces = forces - forces.sum(axis=0) / self._getnatoms()
        h=self.h
        h_inv=np.linalg.inv(h)
